...
export const FLAGS = {
  PERSON: 0b00000001,
  TILE: 0b00000100, // Ô trống/Đường đi
  WALL: 0b00000000,
...
export function generateRandomMazeDFS(width, height) {
  ...
  const carve = (x, y) => {
    grid[getIndex(x, y, width)] |= FLAGS.TILE;
    ...
    for (let { dx, dy } of dirs) {
      ...
      if (nx > 0 && nx < width - 1 && ny > 0 && ny < height - 1) {
        if ((grid[getIndex(nx, ny, width)] & FLAGS.TILE) === 0) {
          grid[getIndex(x + dx / 2, y + dy / 2, width)] |= FLAGS.TILE;
          carve(nx, ny);
        }
      }
    }
  ...
  for (let x = 1; x < width - 1; x += 2) {
    if ((grid[getIndex(x, 1, width)] & FLAGS.TILE) !== 0) possibleExits.push({ x, y: 0 }); 
    if ((grid[getIndex(x, height - 2, width)] & FLAGS.TILE) !== 0) possibleExits.push({ x, y: height - 1 }); 
  }
  // Quét viền trái và phải
  for (let y = 1; y < height - 1; y += 2) {
    if ((grid[getIndex(1, y, width)] & FLAGS.TILE) !== 0) possibleExits.push({ x: 0, y }); 
    if ((grid[getIndex(width - 2, y, width)] & FLAGS.TILE) !== 0) possibleExits.push({ x: width - 1, y }); 
  }
  ...
  for (let i = 0; i < actualExits; i++) {
    ...
    grid[getIndex(exit.x, exit.y, width)] |= FLAGS.TILE;
  }
